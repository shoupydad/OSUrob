...
{
	Ptr = this;
	this->Scope = gcnew Telescope(L"POTH.Telescope");
//	this->Scope = gcnew Telescope(");
...
bool MeadeScope::MoveAxis(int axis, double rate) {
...
bool Scope_BumpArcMin(double delra, double deldec) {
	...
	if (fabs(delra) > 0.0) {
		...
		MeadeScope::Ptr->MoveAxis(0, slewRate);
		usleep(slewTimeInMicroSecs, false);
		MeadeScope::Ptr->MoveAxis(0, 0.0);
	}
	...
	if (fabs(deldec) > 0.0) {
		...
		slewTimeInMicroSecs = (int) (slewTime * 1000000.0);
		MeadeScope::Ptr->MoveAxis(1, slewRate);
		usleep(slewTimeInMicroSecs, false);
		MeadeScope::Ptr->MoveAxis(1, 0.0);
	}
...
bool Scope_Jog(int direction, double rate, bool Start) {
	...
	switch (direction) {
		case NORTH:
			if (Start) {
				MeadeScope::Ptr->MoveAxis(1, rate);
			} else {
				MeadeScope::Ptr->MoveAxis(1, 0.0);
			}
			break;
		...
		case EAST:
			if (Start) {
				MeadeScope::Ptr->MoveAxis(0, rate);
			} else {
				MeadeScope::Ptr->MoveAxis(0, 0.0);
			}
			break;
